...
SELECT v.id,
    ...
    COALESCE(sv.profissional_id, v.vistoriador_id, ( SELECT ri.instalador_id
           FROM rota_instaladores ri
          WHERE ri.rota_id = v.rota_id
         LIMIT 1)) AS vistoriador_id,
    ...
    COALESCE(psv.nome, pv.nome, ( SELECT p.nome
           FROM rota_instaladores ri
             JOIN profiles p ON p.id = ri.instalador_id
          WHERE ri.rota_id = v.rota_id
         LIMIT 1)) AS vistoriador_nome,
  ...
  WHERE (v.status = ANY (ARRAY['pendente'::status_vistoria, 'agendada'::status_vistoria, 'em_rota'::status_vistoria, 'em_andamento'::status_vistoria]))
    ...
    -- DEDUPLICAÇÃO: excluir se já existe serviço standalone para mesmo veículo+data
    AND NOT EXISTS (
      SELECT 1 FROM servicos s2
      WHERE s2.veiculo_id = v.veiculo_id
        AND s2.veiculo_id IS NOT NULL
        AND s2.data_agendada = v.data_agendada
        AND s2.vistoria_origem_id IS NULL
        AND s2.instalacao_origem_id IS NULL
        AND s2.status NOT IN ('cancelada', 'concluida', 'imprevisto_pendente', 'nao_compareceu')
    )

UNION ALL

-- Bloco 2: instalacoes (legado)
SELECT i.id,
    ...
    COALESCE(si.profissional_id, i.instalador_responsavel_id, ( SELECT ri.instalador_id
           FROM rota_instaladores ri
          WHERE ri.rota_id = i.rota_id
         LIMIT 1)) AS vistoriador_id,
    ...
    COALESCE(psi.nome, pi.nome, ( SELECT p.nome
           FROM rota_instaladores ri
             JOIN profiles p ON p.id = ri.instalador_id
          WHERE ri.rota_id = i.rota_id
         LIMIT 1)) AS vistoriador_nome,
...
-- Bloco 3: servicos standalone (canônico)
SELECT s.id,
