...
UPDATE public.instalacao_prestador_links
   SET status = 'cancelada',
       ...
       recusa_motivo = COALESCE(NULLIF(recusa_motivo,''),'') ||
         ...
         '[saneamento_lsa7a65] realocado para fila em 25/05 sem cancelar este link'
 WHERE id = '60110112-3c21-45f0-8389-51ed3c4756f0'
...
SELECT
  i.id, NULL, '2026-05-25'::date, '09:00'::time,
  COALESCE(a.nome,'Cliente'), a.telefone, v.placa,
  trim(COALESCE(v.marca,'') || ' ' || COALESCE(v.modelo,'') || ' ' || COALESCE(v.ano_modelo::text,'')),
  'agendado',
  '[saneamento_lsa7a65] Recriado para devolver a fila do Monitoramento apos realocacao em 25/05 ter cancelado o agendamento anterior sem recriar.'
...
WHERE i.id = 'a5dabcef-f80a-4c4f-88cd-06c3dc80713b'
  AND NOT EXISTS (
    SELECT 1 FROM public.agendamentos_base ab
    WHERE ab.instalacao_id = i.id
      AND ab.status IN ('agendado','confirmado','em_atendimento')
  );

INSERT INTO public.associados_historico (associado_id, tipo, acao, descricao, metadata, usuario_id)
VALUES (
  '26ac0b58-5e09-45cb-9b7b-3fda59e97176',
  'observacao_adicionada',
  'saneamento_lsa7a65',
  '[saneamento_lsa7a65] Link do prestador LEONARDO TORINO MATOSO cancelado e agendamento da fila recriado (realocar_servico nao havia cancelado o link nem recriado o agendamento no destino fila). Cotacao COT-20260521-150643240-471.',
  jsonb_build_object(
    'instalacao_id', 'a5dabcef-f80a-4c4f-88cd-06c3dc80713b',
    'servico_id', '673ac27a-11b9-42b7-ba01-e664330abfd3',
    'link_prestador_id', '60110112-3c21-45f0-8389-51ed3c4756f0',
    'cotacao', 'COT-20260521-150643240-471',
    'placa', 'LSA7A65'
  ),
  NULL
);
