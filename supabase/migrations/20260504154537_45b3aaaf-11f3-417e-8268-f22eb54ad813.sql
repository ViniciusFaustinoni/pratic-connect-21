...
BEGIN
  ...
  -- Status final do serviço/instalação: destino=base cancela o serviço de "rua",
  -- pois a vistoria passa a viver em agendamentos_base (evita duplicação na fila).
  ...
  IF _servico.vistoria_origem_id IS NOT NULL THEN
    ...
    SET data_agendada = _data_alvo, hora_agendada = NULL, updated_at = now()
  ...
  IF _destino = 'base' AND _servico.instalacao_origem_id IS NOT NULL THEN
    ...
    IF _oficina IS NULL THEN RAISE EXCEPTION 'Oficina/base % não encontrada', _oficina_id; END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'rota_nova', _rota_final, 'oficina_nova', _oficina_id, 'oficina_nome', _oficina,
    'nova_data', _data_alvo, 'novo_periodo', _periodo_alvo,
    'agendamentos_fechados', _agendamentos_fechados,
    'instalacao_id', _servico.instalacao_origem_id, 'vistoria_id', _servico.vistoria_origem_id,
    'associado_id', _servico.associado_id, 'veiculo_id', _servico.veiculo_id,
