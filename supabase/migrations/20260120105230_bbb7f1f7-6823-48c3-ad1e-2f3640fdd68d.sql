...
-- Corrigir view_vistorias_mapa para buscar vistoriador_nome pelo profiles.id (não user_id)
...
SELECT 
  ...
  r.cor AS rota_cor,
  -- CORRIGIDO: buscar pelo profiles.id ao invés de profiles.user_id
  COALESCE(
    pv.nome, 
    (SELECT p.nome 
...
LEFT JOIN profiles pv ON pv.id = v.vistoriador_id  -- CORRIGIDO: profiles.id
