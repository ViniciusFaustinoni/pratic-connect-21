-- Limpa o Mapa de Atribuições: só exibir tarefas de hoje em diante
-- Tarefas antigas órfãs (sem conclusão/cancelamento formal) deixam de poluir o mapa.
-- Continuam visíveis em "Serviços de Campo" para tratamento administrativo.
...
SELECT v.id,
    ...
    COALESCE(sv.confirmacao_whatsapp, NULL::text) AS confirmacao_whatsapp,
  ...
  WHERE (v.status = ANY (ARRAY['pendente'::status_vistoria, 'agendada'::status_vistoria, 'em_rota'::status_vistoria, 'em_andamento'::status_vistoria]))
    ...
    AND v.data_agendada::date >= CURRENT_DATE
    AND (sv.id IS NULL OR (sv.status <> ALL (ARRAY['imprevisto_pendente'::status_servico, 'nao_compareceu'::status_servico, 'cancelada'::status_servico, 'concluida'::status_servico])))
    AND NOT (EXISTS (SELECT 1 FROM servicos s2 WHERE s2.veiculo_id = v.veiculo_id AND s2.veiculo_id IS NOT NULL AND s2.data_agendada = v.data_agendada AND s2.vistoria_origem_id IS NULL AND s2.instalacao_origem_id IS NULL AND (s2.status <> ALL (ARRAY['cancelada'::status_servico, 'concluida'::status_servico, 'imprevisto_pendente'::status_servico, 'nao_compareceu'::status_servico]))))
UNION ALL
 SELECT i.id,
    ...
    COALESCE(si.confirmacao_whatsapp, NULL::text) AS confirmacao_whatsapp,
  ...
  WHERE (i.status = ANY (ARRAY['agendada'::status_instalacao, 'em_rota'::status_instalacao, 'em_andamento'::status_instalacao, 'reagendada'::status_instalacao]))
    ...
    AND i.data_agendada::date >= CURRENT_DATE
    AND (si.id IS NULL OR (si.status <> ALL (ARRAY['imprevisto_pendente'::status_servico, 'nao_compareceu'::status_servico, 'cancelada'::status_servico, 'concluida'::status_servico])))
    AND NOT (EXISTS (SELECT 1 FROM servicos s2 WHERE s2.veiculo_id = i.veiculo_id AND s2.veiculo_id IS NOT NULL AND s2.data_agendada = i.data_agendada AND s2.vistoria_origem_id IS NULL AND s2.instalacao_origem_id IS NULL AND (s2.status <> ALL (ARRAY['cancelada'::status_servico, 'concluida'::status_servico, 'imprevisto_pendente'::status_servico, 'nao_compareceu'::status_servico]))))
UNION ALL
 SELECT s.id,
  ...
  WHERE (s.status = ANY (ARRAY['pendente'::status_servico, 'agendada'::status_servico, 'em_rota'::status_servico, 'em_andamento'::status_servico]))
    ...
    AND s.data_agendada::date >= CURRENT_DATE;