...
BEGIN
  ...
  -- DEDUPE: ao realocar para Base, fechar TODOS agendamentos ativos do mesmo
  -- veículo/telefone (cobre órfãos sem instalacao_id)
  IF _destino = 'base' THEN
    UPDATE public.agendamentos_base
    SET status = 'cancelado', updated_at = now(),
        observacoes = COALESCE(NULLIF(observacoes,''), '') ||
...
-- BACKFILL: cancelar duplicatas ativas (manter apenas a mais recente por placa/telefone)
WITH ranked AS (
  SELECT id,
         row_number() OVER (
           PARTITION BY upper(trim(coalesce(veiculo_placa,''))), regexp_replace(coalesce(cliente_telefone,''),'\D','','g')
           ORDER BY created_at DESC NULLS LAST, id DESC
         ) AS rn
  FROM public.agendamentos_base
  WHERE status IN ('agendado','confirmado','em_atendimento')
    AND (coalesce(veiculo_placa,'') <> '' OR coalesce(cliente_telefone,'') <> '')
)
UPDATE public.agendamentos_base ab
SET status = 'cancelado', updated_at = now(),
    observacoes = COALESCE(NULLIF(observacoes,''), '') ||
      CASE WHEN COALESCE(observacoes,'') = '' THEN '' ELSE E'\n' END ||
      '[Backfill dedupe ' || to_char(now(),'YYYY-MM-DD') || '] Duplicidade cancelada (regra dedupe-agendamentos-rule)'
FROM ranked r
