-- Recria view_acompanhamento sem dependência de cotacoes_publicas
-- (removida junto da jornada legada /q/:token).
-- Mantém o shape de colunas para não quebrar o tipo TS AcompanhamentoItem.

DROP VIEW IF EXISTS view_acompanhamento;

CREATE OR REPLACE VIEW view_acompanhamento
...
SELECT
  ...
  NULL::uuid AS cotacao_publica_id,
  NULL::text AS cotacao_publica_status,
...
GRANT SELECT ON view_acompanhamento TO authenticated;
GRANT SELECT ON view_acompanhamento TO service_role;