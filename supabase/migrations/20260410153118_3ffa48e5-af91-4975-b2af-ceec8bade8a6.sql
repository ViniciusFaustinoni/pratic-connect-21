...
-- Recreate view to exclude services with terminal status
...
WHERE v.status IN ('pendente','agendada','em_rota','em_andamento')
  ...
  AND (v.data_agendada IS NULL OR v.data_agendada >= CURRENT_DATE - interval '30 days')
...
WHERE i.status IN ('agendada','em_rota','em_andamento','reagendada')
  ...
  AND (i.data_agendada IS NULL OR i.data_agendada >= CURRENT_DATE - interval '30 days')
  AND (si.id IS NULL OR si.status NOT IN ('imprevisto_pendente','nao_compareceu','cancelada','concluida'))

UNION ALL

-- SERVICOS DIRETOS (sem vistoria/instalacao de origem)
SELECT s.id,
...
WHERE s.status IN ('pendente','agendada','em_rota','em_andamento')
  ...
  AND (s.data_agendada IS NULL OR s.data_agendada >= CURRENT_DATE - interval '30 days');