...
-- Adicionar colunas de contato prévio na tabela servicos
ALTER TABLE public.servicos ADD COLUMN IF NOT EXISTS contato_realizado_em TIMESTAMPTZ DEFAULT NULL;
ALTER TABLE public.servicos ADD COLUMN IF NOT EXISTS contato_tipo TEXT DEFAULT NULL;

-- Atualizar RPC para incluir novos campos
...
CREATE OR REPLACE FUNCTION public.buscar_tarefa_atual_profissional(p_profissional_id uuid)
 RETURNS TABLE(
   ...
   em_rota_em timestamp with time zone, 
   instalacao_origem_id uuid, 
   vistoria_origem_id uuid, 
   confirmacao_whatsapp text, 
   confirmado_via_whatsapp_em timestamp with time zone,
...
'Retorna a tarefa atual do profissional, incluindo campos contato_realizado_em e contato_tipo. v4 - 2026-02-10';
