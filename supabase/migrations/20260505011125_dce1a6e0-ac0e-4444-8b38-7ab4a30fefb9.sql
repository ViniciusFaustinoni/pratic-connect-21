...
-- Índices para paginação e contadores
CREATE INDEX IF NOT EXISTS idx_cotacoes_vendedor_created
  ON public.cotacoes (vendedor_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_cotacoes_status_contratacao
  ON public.cotacoes (status, status_contratacao);

-- Função que retorna contadores do funil sem trazer linhas
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'concluido', COUNT(*) FILTER (
      WHERE status = 'aceita' OR status_contratacao = 'concluido'
    ),
    'perdida', COUNT(*) FILTER (WHERE status IN ('recusada','expirada'))
  )
  INTO v_result
  FROM base;
...
GRANT EXECUTE ON FUNCTION public.cotacoes_funil_counts(uuid, text, text) TO authenticated;
