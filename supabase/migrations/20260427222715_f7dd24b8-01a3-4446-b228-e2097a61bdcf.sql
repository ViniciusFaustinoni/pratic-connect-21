...
BEGIN
  ...
  FROM (
    ...
    WHERE s.profissional_id = p_profissional_id
      -- Correção: 'em_analise' removido. Após a submissão pelo técnico, o
      -- serviço aguarda revisão do gestor e NÃO deve mais aparecer como
      -- "tarefa atual" no app do instalador.
    ...
    WHERE ab.tecnico_id = p_profissional_id
