-- Corrige public.realocar_servico: referência a tabela inexistente `oficinas_credenciadas`.
-- A tabela real chama-se `public.oficinas` e o nome humano fica em `nome_fantasia`
-- (com fallback para `razao_social`). Sem essa correção, realocar para destino='base'
-- falhava com: relation "public.oficinas_credenciadas" does not exist.
...
BEGIN
  ...
  IF _destino = 'base' AND _servico.instalacao_origem_id IS NOT NULL THEN
    ...
    -- CORREÇÃO RAIZ: tabela é public.oficinas (não oficinas_credenciadas).
    -- O nome humano fica em nome_fantasia, com fallback para razao_social.
