...
serve(async (req) => {
  ...
  try {
    ...
    // Verificar flag de atribuição manual — quando ativa, todo o motor automático é desligado
    ...
    if (configManual?.valor === 'true') {
      console.log('[cron-atribuir-tarefas] Atribuição MANUAL ativa — atribuição automática desligada');
      return new Response(
    ...
    const tempoQuaseDisponivelMin = await getConfiguracaoNumero(supabase, 'fila_tempo_quase_disponivel_min', 75);
    const etapaQuaseDisponivel = await getConfiguracaoNumero(supabase, 'fila_etapa_quase_disponivel', 4);
