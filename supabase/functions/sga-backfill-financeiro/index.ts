...
serve(async (req) => {
  ...
  try {
    ...
    if (acao === 'status') {
      ...
      const { count: veiculosSemCodigo } = await supabase
        .from('veiculos')
        .select('id', { count: 'exact', head: true })
        .is('codigo_hinova', null);
      const { count: veiculosComCodigo } = await supabase
        .from('veiculos')
        .select('id', { count: 'exact', head: true })
        .not('codigo_hinova', 'is', null);
      return json(200, { success: true, jobs: counts, veiculos_sem_codigo: veiculosSemCodigo ?? 0, veiculos_com_codigo: veiculosComCodigo ?? 0 });
    }

    // -------- ENFILEIRAR --------
    if (acao === 'enfileirar') {
      // Veículos com codigo_hinova preenchido e associado com codigo_hinova
      // que ainda não tenham um job pendente/executando/concluído ativo de mesmo tipo nas últimas 24h.
      const pageSize = 1000;
      let from = 0;
      let inseridos = 0;
      while (true) {
        const { data: vs, error } = await supabase
          .from('veiculos')
          .select('id, associado_id, associados:associados!inner(codigo_hinova)')
          .not('codigo_hinova', 'is', null)
