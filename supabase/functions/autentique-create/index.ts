  ...
  try {
    ...
    if (!templateDB && isSubstituicao) {
      ...
      if (subDefault) {
        ...
        console.log(`[autentique-create] Usando template default de SUBSTITUIÇÃO: ${subDefault.codigo} (${subDefault.nome})`);
      } else {
        console.warn('[autentique-create] Cotação é de substituição mas nenhum template com is_default_substituicao=true foi encontrado; caindo no default de filiação');
      }
    }
    ...
    // ============= BUSCAR VEÍCULO DO BANCO (FLAGS DE DEPRECIAÇÃO + CÂMBIO/TIPO + COMBUSTÍVEL + FIPE) =============
    ...
    // ============= BUSCAR COTAÇÃO (fallback de combustível / FIPE / etc.) =============
    ...
    // Fonte canônica em CASCATA porque substituicoes_veiculo só é materializada
    // no efetivar-substituicao (pós retirada+instalação), e o termo nasce ANTES:
    //   1) substituicoes_veiculo (já materializada)
    //   2) cotacoes.dados_extras.veiculo_antigo_* (gravado na criação da cotação)
    //   3) solicitacoes_substituicao_placa (via dados_extras.solicitacao_substituicao_id ou cotacao_id)
    //   4) tabela veiculos via veiculo_antigo_id (best-effort para modelo/fipe)
    if ((contrato.tipo_entrada === 'substituicao_placa' || contrato.tipo_entrada === 'substituicao') && contrato.associado_id) {
      ...
      if (placaAnterior) {
        ...
        console.log(`[autentique-create] Dados de substituição via ${fonte}:`, placaAnterior, modeloAnterior || '(sem modelo)');
      } else {
        console.warn('[autentique-create] Contrato de substituição SEM placa anterior resolvida (nenhuma fonte) — tokens cairão para "—"');
      }
    }
    ...
    logEdgeFunction({ functionName: "autentique-create", plataforma: "autentique", operacao: "create", status: "sucesso", tempoMs: Date.now() - _startTime });
