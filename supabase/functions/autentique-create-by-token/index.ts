    ...
    if (planoId) {
      try {
        ...
        if (coberturasData?.length) {
          ...
          console.log(`[autentique-create-by-token] ${coberturasData.length} coberturas carregadas`);
        }
        ...
        if (beneficiosData?.length) {
          ...
          console.log(`[autentique-create-by-token] ${beneficiosData.length} benefícios carregados`);
        }
      } catch (err) {
    ...
    try {
      ...
      if (signatureLink && telefoneWpp) {
        ...
        const wppRes = await fetch(`${SUPABASE_URL}/functions/v1/whatsapp-send-text`, {
          ...
          body: JSON.stringify({ telefone: telefoneWpp, template_name: 'assinatura_documento_v2', template_params: [clienteNome, nomeDoc], template_button_params: [linkCode] }),
        });
        if (wppRes.ok) {
