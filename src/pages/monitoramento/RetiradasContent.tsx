...
export default function RetiradasContent() {
  ...
  const retiradasFiltradas = useMemo(() => {
    ...
    if (statusFilter !== 'todos') result = result.filter(r => r.status === statusFilter);
    if (motivoFilter !== 'todos') result = result.filter(r => r.motivo_retirada === motivoFilter);
    if (origemFilter !== 'todas') result = result.filter(r => r.solicitado_por_modulo === origemFilter);
    return result;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
        <Card className="cursor-pointer hover:bg-muted/50 transition-colors" onClick={() => setStatusFilter('pendente')}>
          ...
          <CardContent><div className="text-2xl font-bold">{metricas.pendentes}</div></CardContent>
        </Card>
        <Card className="cursor-pointer hover:bg-muted/50 transition-colors" onClick={() => setStatusFilter('agendada')}>
          <CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">Agendadas</CardTitle></CardHeader>
          <CardContent><div className="text-2xl font-bold text-blue-600">{metricas.agendadas}</div></CardContent>
        </Card>
        <Card className="cursor-pointer hover:bg-muted/50 transition-colors" onClick={() => setStatusFilter('em_rota')}>
          <CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">Em Campo</CardTitle></CardHeader>
          <CardContent><div className="text-2xl font-bold text-purple-600">{metricas.emCampo}</div></CardContent>
        </Card>
        <Card className="cursor-pointer hover:bg-muted/50 transition-colors" onClick={() => setStatusFilter('concluida')}>
          ...
          <CardContent><div className="text-2xl font-bold text-green-600">{metricas.concluidas}</div></CardContent>
        </Card>
        <Card className="cursor-pointer hover:bg-muted/50 transition-colors" onClick={() => setStatusFilter('nao_compareceu')}>
          <CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">Não Compareceu</CardTitle></CardHeader>
          <CardContent><div className="text-2xl font-bold text-orange-600">{metricas.naoCompareceu}</div></CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground flex items-center gap-1"><DollarSign className="h-3 w-3" />Com Multa</CardTitle></CardHeader>
          <CardContent><div className="text-2xl font-bold text-red-600">{metricas.comMulta}</div></CardContent>
        </Card>
        <Card className="cursor-pointer hover:bg-muted/50 transition-colors border-orange-200" onClick={() => setOrigemFilter('cadastro')}>
          <CardHeader className="pb-2"><CardTitle className="text-sm text-orange-600 flex items-center gap-1"><Lock className="h-3 w-3" />Do Cadastro</CardTitle></CardHeader>
          <CardContent><div className="text-2xl font-bold text-orange-600">{metricas.doCadastro}</div></CardContent>
        </Card>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          ...
          <Input placeholder="Buscar por nome, placa ou código..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-10" />
        </div>
        ...
        {(search || statusFilter !== 'todos' || motivoFilter !== 'todos' || origemFilter !== 'todas') && (
          <Button variant="ghost" size="sm" onClick={limparFiltros}><X className="h-4 w-4 mr-1" />Limpar</Button>
        )}
      </div>
            ...
            {isLoading ? (
              ...
              retiradasFiltradas.map((retirada) => {
                ...
                return (
                  ...
                  >
                    <TableCell className="font-mono text-sm">{retirada.protocolo || '-'}</TableCell>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <User className="h-4 w-4 text-muted-foreground" />
                        <span className="font-medium truncate max-w-[150px]">{retirada.associado?.nome || '-'}</span>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <div className="text-sm">{retirada.veiculo?.marca} {retirada.veiculo?.modelo}</div>
                          <div className="text-xs text-muted-foreground font-mono">{retirada.veiculo?.placa}</div>
                        </div>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-mono text-sm">{retirada.rastreador?.codigo || '-'}</span>
                      </div>
                    </TableCell>
                    <TableCell>
                      {retirada.motivo_retirada ? (
                        <Badge variant="outline" className="text-xs">{MOTIVO_RETIRADA_LABELS[retirada.motivo_retirada]}</Badge>
                      ) : '-'}
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>{retirada.profissional?.nome || <span className="text-muted-foreground text-sm">-</span>}</TableCell>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <Badge className={cn('text-xs', statusConfig.className)}>{statusConfig.label}</Badge>
                        {retirada.multa_aplicada && (
                          <Badge variant="destructive" className="text-xs"><DollarSign className="h-3 w-3 mr-0.5" />Multa</Badge>
                        )}
                    ...
                    <TableCell>
                      {retirada.solicitado_por_modulo === 'cadastro' ? (
                        <Badge variant="outline" className="text-xs border-orange-300 text-orange-700"><Lock className="h-3 w-3 mr-0.5" />Cadastro</Badge>
                      ) : (
                        <span className="text-xs text-muted-foreground capitalize">{retirada.solicitado_por_modulo || 'monitoramento'}</span>
                      )}
                    </TableCell>
                    <TableCell>
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          {retirada.status === 'pendente' && (
                            <DropdownMenuItem onClick={() => handleAgendar(retirada)}><Calendar className="h-4 w-4 mr-2" />Agendar</DropdownMenuItem>
                          )}
                          {retirada.status === 'agendada' && (
                            <DropdownMenuItem onClick={() => handleAgendar(retirada)}><RefreshCw className="h-4 w-4 mr-2" />Reagendar</DropdownMenuItem>
                          )}
                          {retirada.status === 'nao_compareceu' && (
                            <DropdownMenuItem onClick={() => handleTratarAusencia(retirada)}><AlertTriangle className="h-4 w-4 mr-2" />Tratar Ausência</DropdownMenuItem>
                          )}
                          <DropdownMenuSeparator />
                          <DropdownMenuItem onClick={() => handleAplicarMulta(retirada)}><DollarSign className="h-4 w-4 mr-2" />Aplicar Multa</DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
                  </TableRow>
      ...
      <TratarAusenciaRetiradaModal
        ...
        onClose={() => { setTratarAusenciaOpen(false); setSelectedRetirada(null); }}
        retirada={selectedRetirada}
      />
      {selectedRetirada && (
        <AplicarMultaModal
          open={aplicarMultaOpen}
          onOpenChange={(open) => { setAplicarMultaOpen(open); if (!open) setSelectedRetirada(null); }}
          retirada={{
            id: selectedRetirada.id,
            associado: selectedRetirada.associado ? { nome: selectedRetirada.associado.nome, cpf: selectedRetirada.associado.cpf || '' } : null,
            rastreador: selectedRetirada.rastreador ? { codigo: selectedRetirada.rastreador.codigo } : null,
      ...
      <AgendarRetiradaModal
        ...
        onOpenChange={(open) => { setAgendarModalOpen(open); if (!open) setSelectedRetirada(null); }}
        retirada={selectedRetirada}
      />
    </div>
  );
