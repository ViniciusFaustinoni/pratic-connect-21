...
export default function Mapa() {
  const [abaAtiva, setAbaAtiva] = useState<string>("associados");
  ...
  const renderMapaVeiculos = () => (
    <MapContainer center={centroInicial} zoom={4} className="h-full w-full" style={{ height: "100%", width: "100%" }}>
      ...
      {veiculoSelecionado && veiculoSelecionado.latitude && veiculoSelecionado.longitude && (() => {
        ...
        return (
          <Marker key={v.rastreador_id} position={[v.latitude!, v.longitude!]} icon={icon}>
            <Popup>
              <div className="min-w-[200px]">
                ...
                <div className="flex items-center gap-3 text-xs mb-2">
                  <span className="flex items-center gap-1"><Gauge className="h-3 w-3" />{v.velocidade || 0} km/h</span>
                  <span className="flex items-center gap-1"><Power className="h-3 w-3" />{v.ignicao ? "Ligado" : "Desligado"}</span>
                </div>
                {v.ultima_comunicacao && (
                  <p className="text-xs text-gray-500 mb-3">
                    Última comunicação: {format(new Date(v.ultima_comunicacao), "dd/MM/yyyy HH:mm", { locale: ptBR })}
                  </p>
                )}
                <div className="flex gap-2 flex-wrap">
                  ...
                  <button onClick={() => carregarTrajeto(v.rastreador_id)} disabled={trajetoLoading} className={`flex items-center gap-1 px-3 py-1.5 rounded text-xs text-white ${trajetoAtivo === v.rastreador_id ? 'bg-amber-600 hover:bg-amber-700' : 'bg-purple-600 hover:bg-purple-700'}`}>
                    <Route className="h-3 w-3" />{trajetoAtivo === v.rastreador_id ? 'Fechar Trajeto' : 'Ver Trajeto'}
                  </button>
                  <button onClick={() => atualizarPosicao.mutate(v.rastreador_id)} disabled={atualizarPosicao.isPending} className="flex items-center gap-1 px-3 py-1.5 bg-gray-600 text-white rounded text-xs hover:bg-gray-700">
                    <RefreshCw className={`h-3 w-3 ${atualizarPosicao.isPending ? 'animate-spin' : ''}`} />Atualizar
                  </button>
                </div>
              </div>
            </Popup>
      ...
      {trajetoAtivo && pontosTrajetoAtivo.length > 1 && <TrajetoOSRM pontos={pontosTrajetoAtivo} />}
    </MapContainer>
  );
    ...
    return (
      <div className="absolute top-16 right-4 z-[400] bg-background/95 backdrop-blur-sm px-3 py-2 rounded-lg border shadow-sm text-xs space-y-1">
        <div className="flex items-center gap-2 font-medium">
          <Route className="h-3.5 w-3.5 text-purple-500" />Trajeto (4h)
        </div>
        ...
        <button onClick={() => { setTrajetoAtivo(null); setPontosTrajetoAtivo([]); }} className="text-xs text-muted-foreground hover:text-foreground underline">
          Fechar trajeto
        </button>
      </div>
    );
    ...
    <div ref={searchRef} className="absolute top-4 left-4 right-4 md:left-4 md:right-auto md:w-96 z-[400]">
      <div className="relative">
        ...
        {(termoBusca || veiculoSelecionado) && (
          <button onClick={limparSelecao} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground">
            <X className="h-4 w-4" />
          </button>
        )}
      </div>
      {showResultados && resultadosBusca.length > 0 && (
        <div className="mt-1 bg-background/95 backdrop-blur-sm border rounded-lg shadow-lg max-h-72 overflow-y-auto">
          {resultadosBusca.map((v) => (
            <button key={v.rastreador_id} className="w-full text-left px-4 py-3 hover:bg-muted/50 border-b last:border-b-0 transition-colors" onClick={() => selecionarVeiculo(v)}>
              <div className="flex items-center justify-between">
                <div>
                  <span className="font-semibold text-sm">{v.placa || "Sem placa"}</span>
                  <span className="text-xs text-muted-foreground ml-2">{v.marca} {v.modelo}</span>
                </div>
                <span className="text-xs">{getStatusLabel(v.status_comunicacao)}</span>
              </div>
    ...
    <div className="flex flex-col h-[calc(100dvh-4rem)] p-2 md:p-4">
      <Tabs value={abaAtiva} onValueChange={setAbaAtiva} className="flex flex-col h-full">
        <TabsList className="w-fit mb-2 md:mb-4">
          <TabsTrigger value="associados" className="gap-2 text-xs md:text-sm">
        ...
        {/* Aba Associados */}
        <TabsContent value="associados" className="flex-1 mt-0">
