...
import { Search, Plus, Download, Loader2, MapPin, ChevronRight } from 'lucide-react';
...
const MUNICIPIOS_RJ: { nome: string; tipo: TipoAtendimento }[] = [
  ...['Duque de Caxias','Rio de Janeiro','São João de Meriti','Belford Roxo','Nilópolis','Mesquita','Nova Iguaçu','Queimados','Japeri','Paracambi','Seropédica','Itaguaí','Magé','Guapimirim','Petrópolis','Teresópolis','Niterói','São Gonçalo','Maricá','Itaboraí','Tanguá','Rio Bonito','Cachoeiras de Macacu','Miguel Pereira','Paty do Alferes','Mendes','Engenheiro Paulo de Frontin'].map(nome => ({ nome, tipo: 'volante' as TipoAtendimento })),
  ...['Nova Friburgo','Sumidouro','São José do Vale do Rio Preto','Vassouras','Barra do Piraí','Piraí','Rio Claro','Valença','Barra Mansa','Volta Redonda','Quatis','Porto Real','Resende','Itatiaia','Angra dos Reis','Mangaratiba','Paraty','Carmo','Saquarema','Silva Jardim'].map(nome => ({ nome, tipo: 'viagem' as TipoAtendimento })),
  ...['Araruama','Iguaba Grande','São Pedro da Aldeia','Cabo Frio','Arraial do Cabo','Armação dos Búzios','Casimiro de Abreu','Rio das Ostras','Macaé','Carapebus','Quissamã','Campos dos Goytacazes','São João da Barra'].map(nome => ({ nome, tipo: 'prestador' as TipoAtendimento })),
  ...['Itaperuna','Bom Jesus do Itabapoana','Natividade','Porciúncula','Varre-Sai','Laje do Muriaé','Miracema','Santo Antônio de Pádua','Aperibé','Cambuci','Itaocara','São Fidélis','Cardoso Moreira','São Francisco de Itabapoana','Italva','Cantagalo','Cordeiro','Macuco','Trajano de Moraes','Santa Maria Madalena','São Sebastião do Alto','Duas Barras','Bom Jardim','Conceição de Macabu'].map(nome => ({ nome, tipo: 'fora_cobertura' as TipoAtendimento })),
];

export default function MapaAtendimentoPage() {
  ...
  const [openUfs, setOpenUfs] = useState<Set<string>>(new Set());
  ...
  const importMutation = useMutation({
    mutationFn: async () => {
      const rows = MUNICIPIOS_RJ.map(m => ({ nome: m.nome, uf: 'RJ', tipo_atendimento: m.tipo }));
  ...
  const { data: configViagem } = useQuery({
    ...
    queryFn: async () => {
      ...
      return { diaria: map.viagem_valor_diaria || '0', sla: map.viagem_sla_horas || '72', prestadorHoras: map.prestador_horas_alerta_sem_resposta || '2' };
    },
    staleTime: 1000 * 60 * 5,
  });
  ...
  return (
    <div className="space-y-6 p-6">
      <Breadcrumb>
        <BreadcrumbList>
          <BreadcrumbItem><BreadcrumbLink href="/monitoramento/dashboard">Monitoramento</BreadcrumbLink></BreadcrumbItem>
          <BreadcrumbSeparator />
          <BreadcrumbItem><BreadcrumbPage>Regiões de Atuação</BreadcrumbPage></BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>

      <div>
        <h1 className="text-2xl font-bold tracking-tight">Regiões de Atuação</h1>
        <p className="text-sm text-muted-foreground mt-1">Classifique municípios por tipo de atendimento operacional</p>
      </div>
        ...
        <Card>
          <CardContent className="py-12 text-center space-y-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1">Importe a classificação padrão do Rio de Janeiro para começar.</p>
            </div>
        ...
        <>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <Input placeholder="Buscar município..." value={busca} onChange={e => setBusca(e.target.value)} className="pl-9" />
            </div>
          ...
          <div className="space-y-2">
            {groupedByUf.map(([uf, items]) => (
              <Collapsible key={uf} open={openUfs.has(uf)} onOpenChange={() => toggleUf(uf)}>
                <CollapsibleTrigger asChild>
                  <button className="w-full flex items-center gap-3 px-4 py-3 rounded-lg border bg-card hover:bg-muted/50 transition-colors">
                    <ChevronRight className={cn('h-4 w-4 text-muted-foreground transition-transform', openUfs.has(uf) && 'rotate-90')} />
                    <span className="font-semibold text-sm">{uf}</span>
                    <Badge variant="secondary" className="text-xs">{items.length} municípios</Badge>
                  </button>
                </CollapsibleTrigger>
                <CollapsibleContent>
                  <div className="ml-4 mt-1 border-l pl-4 space-y-1">
                    {items.map(m => (
                      <div key={m.id} className="flex items-center justify-between py-2 px-3 rounded hover:bg-muted/30">
            ...
            {groupedByUf.length === 0 && (
              <p className="text-center text-muted-foreground py-8 text-sm">Nenhum município encontrado</p>
            )}
          </div>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  ...
                  <Input type="number" min="0" step="0.01" value={viagemDiaria} onChange={e => setViagemDiaria(e.target.value)} placeholder="0" />
                  <p className="text-xs text-muted-foreground mt-1">Zero = sem compensação</p>
                </div>
                <div>
                  <Label>SLA de instalação em viagem (horas úteis)</Label>
                  <Input type="number" min="1" value={viagemSla} onChange={e => setViagemSla(e.target.value)} placeholder="72" />
                  <p className="text-xs text-muted-foreground mt-1">Padrão volante: 48h</p>
                </div>
                <div>
                  <Label>Horas sem resposta para alerta (prestador)</Label>
                  <Input type="number" min="1" value={prestadorHorasAlerta} onChange={e => setPrestadorHorasAlerta(e.target.value)} placeholder="2" />
                  <p className="text-xs text-muted-foreground mt-1">Alerta ao coordenador após X horas</p>
                </div>
              </div>
              <Button size="sm" onClick={() => salvarViagemMutation.mutate()} disabled={salvarViagemMutation.isPending}>
      ...
      <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
        <DialogContent>
          <DialogHeader><DialogTitle>Adicionar Município</DialogTitle></DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button onClick={() => addMutation.mutate({ nome: novoNome.trim(), uf: novoUf.trim(), tipo_atendimento: novoTipo })} disabled={!novoNome.trim() || addMutation.isPending}>
