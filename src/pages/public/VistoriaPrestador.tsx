...
import { MapPin, Phone, Camera, CheckCircle, Clock, AlertTriangle, Loader2, Upload, XCircle } from 'lucide-react';
...
export default function VistoriaPrestador() {
  ...
  const { data: link, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Check if concluded or cancelled
      if ((data as any).status === 'cancelada') {
        throw new Error('Esta tarefa foi cancelada.');
      }
  ...
  const concluir = useMutation({
    mutationFn: async () => {
      ...
      for (const foto of fotos) {
        ...
        const path = `vistoria-prestador/${link.id}/${Date.now()}_${Math.random().toString(36).slice(2)}.${ext}`;
  ...
  if (error || !link) {
    ...
    const isCancelled = errMsg.includes('cancelada');
        ...
        <Card className="w-full max-w-md">
          <CardContent className="pt-6 text-center">
            ...
            <h2 className="text-lg font-semibold mb-2">
              {isCancelled ? 'Tarefa cancelada' : 'Link inválido ou expirado'}
            </h2>
            <p className="text-muted-foreground text-sm">
              {isCancelled
                ? 'Esta tarefa de vistoria foi cancelada. Entre em contato com a Praticcar para mais informações.'
                : 'Este link não existe ou já foi utilizado. Entre em contato com a Praticcar.'}
            </p>
          </CardContent>
        </Card>
      </div>
    ...
    <div className="min-h-screen bg-background p-4 pb-24">
      <div className="max-w-md mx-auto space-y-4">
        ...
        <div className="text-center pt-2 pb-4">
          <h1 className="text-xl font-bold">Vistoria Praticcar</h1>
        ...
        {link.status === 'concluida' && (
          <Card className="border-green-500/30">
            <CardContent className="pt-6 text-center">
              ...
              <h2 className="text-lg font-semibold text-green-700">Vistoria concluída!</h2>
