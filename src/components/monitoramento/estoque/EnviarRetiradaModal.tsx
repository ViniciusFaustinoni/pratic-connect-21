...
import { Calendar as CalendarIcon, PackageMinus, AlertTriangle, Loader2 } from 'lucide-react';
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
import { useCriarRetirada } from '@/hooks/useCriarRetirada';
...
interface RastreadorParaRetirada {
  id: string;
  codigo: string;
  imei?: string | null;
...
interface EnviarRetiradaModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  rastreador: RastreadorParaRetirada | null;
}
...
export function EnviarRetiradaModal({ 
  open, 
  onOpenChange, 
  rastreador 
}: EnviarRetiradaModalProps) {
  const [dataSelecionada, setDataSelecionada] = useState<Date | undefined>();
  const [periodoSelecionado, setPeriodoSelecionado] = useState<Periodo | null>(null);
  const [motivo, setMotivo] = useState('');

  const { mutate: criarRetirada, isPending } = useCriarRetirada();
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Agende a retirada deste rastreador. Um vistoriador será atribuído para executar o serviço.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              id="motivo"
              placeholder="Ex: Cliente solicitou cancelamento, veículo vendido..."
              value={motivo}
          ...
          <div className="flex items-start gap-2 p-3 rounded-md bg-red-50 dark:bg-red-950/30 text-red-800 dark:text-red-200">
            ...
            <p className="text-xs">
              Após a retirada, o rastreador será desativado na plataforma, desvinculado do veículo 
              e retornará ao estoque do vistoriador que executou a tarefa.
            </p>
          </div>
        </div>
