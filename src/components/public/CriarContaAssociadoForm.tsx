...
import { Eye, EyeOff, KeyRound, Loader2, Rocket, Check, X } from 'lucide-react';
...
export function CriarContaAssociadoForm({ associadoId, nomeAssociado }: CriarContaAssociadoFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('app-criar-conta-cliente', {
        body: { associadoId, email: email.toLowerCase().trim(), senha }
      });
