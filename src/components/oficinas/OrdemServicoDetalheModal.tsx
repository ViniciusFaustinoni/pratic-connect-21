...
import { Building2, Car, User, Calendar, FileText, Plus, Trash2 } from 'lucide-react';
...
interface OrdemServicoDetalheModalProps {
  osId: string | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function OrdemServicoDetalheModal({ osId, open, onOpenChange }: OrdemServicoDetalheModalProps) {
  const [statusOpen, setStatusOpen] = useState(false);
  const [itemOpen, setItemOpen] = useState(false);
  const [conclusaoOpen, setConclusaoOpen] = useState(false);

  const { data: os, isLoading } = useOrdemServico(osId ?? undefined);
  const { data: itens } = useOSItens(osId ?? undefined);
  const { data: historico } = useOSHistorico(osId ?? undefined);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="sr-only">Detalhe da Ordem de Serviço</DialogTitle>
        </DialogHeader>
            ...
            <div className="flex flex-wrap items-center gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 flex-wrap">
                  <h2 className="text-2xl font-bold">{os.numero}</h2>
                  ...
                  <Card>
                    <CardContent className="flex items-center gap-4 p-4">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="font-medium truncate">{os.oficina?.nome_fantasia || os.oficina?.razao_social}</p>
                      </div>
                    </CardContent>
                  </Card>
                  <Card>
                    <CardContent className="flex items-center gap-4 p-4">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="font-medium truncate">{os.associado?.nome}</p>
                      </div>
                    </CardContent>
                  </Card>
                  <Card>
                    <CardContent className="flex items-center gap-4 p-4">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="font-medium truncate">{os.veiculo?.placa}</p>
                      </div>
                    </CardContent>
                  </Card>
                </div>
