...
export function PlanFormModal({
  ...
  useEffect(() => {
    ...
    if (planId) {
      ...
      const nextKey = `${planId}:${fullPlanData.updated_at ?? fullPlanData.id}`;
      if (hydratedStateKeyRef.current === nextKey) return;
      ...
      hydratedStateKeyRef.current = nextKey;
      return;
    }

    const nextKey = `new:${defaultProductLineId ?? ''}`;
    if (hydratedStateKeyRef.current === nextKey) return;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="h-[92vh] max-w-7xl overflow-hidden p-0">
        ...
        <div className="grid h-[calc(92vh-77px)] min-h-0 xl:grid-cols-[minmax(0,1fr)_320px]">
          <div className="min-h-0 border-b border-border/60 xl:border-b-0 xl:border-r xl:border-r-border/60">
            {isEditing && loadingPlan && !fullPlanData ? (
