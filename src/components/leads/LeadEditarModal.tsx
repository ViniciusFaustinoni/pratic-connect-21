...
import { Save, User, Car, Settings, AlertCircle } from 'lucide-react';
...
const editarLeadSchema = z.object({
  ...
  veiculo_ano: z.string().optional().refine(
  ...
  if (data.etapa === 'perdido') return data.motivo_perda && data.motivo_perda.length >= 10;
  return true;
}, {
...
const maskPlaca = (value: string) =>
  value.toUpperCase().replace(/[^A-Z0-9]/g, '').replace(/^([A-Z]{3})(\d)/, '$1-$2').substring(0, 8);
...
interface LeadEditarModalProps {
  leadId: string | null;
  open: boolean;
  onClose: () => void;
}

export function LeadEditarModal({ leadId, open, onClose }: LeadEditarModalProps) {
  const { data: lead, isLoading, error } = useLead(leadId || undefined);
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
      <DialogContent className="max-w-3xl w-[95vw] max-h-[95vh] p-0 gap-0 flex flex-col overflow-hidden">
