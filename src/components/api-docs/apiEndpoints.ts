...
export const apiEndpoints: ApiEndpoint[] = [
  // ASSOCIADOS
  {
    ...
    fields: [
      ...
      { name: 'cnh_validade', type: 'string (YYYY-MM-DD)', required: false, description: 'Data de vencimento da CNH' },
      { name: 'cnh_numero', type: 'string', required: false, description: 'Número de registro da CNH' },
      { name: 'cnh_categoria', type: 'string', required: false, description: 'Categoria da CNH (A, B, AB, etc.)' },
      { name: 'data_cadastro_sga', type: 'string (YYYY-MM-DD)', required: false, description: 'Data de cadastro no SGA' },
  ...
  },
  {
    ...
    responseExample: {
      ...
      cnh_numero: '12345678900',
      cnh_categoria: 'AB',
      data_cadastro_sga: '2025-06-01',
      data_contrato: '2026-01-15',
      data_contrato_final: '2027-01-15',
      plano_id: '...',
      cidade: 'São Paulo',
      uf: 'SP',
    },
  ...
  // VEÍCULOS
  {
    ...
    fields: [
      ...
      { name: 'valor_fipe_protegido', type: 'number', required: false, description: 'Valor FIPE protegido (valor efetivamente coberto)' },
  ...
  // SINISTROS / EVENTOS
  {
    ...
    fields: [
      ...
      { name: 'local_ocorrencia', type: 'string', required: false, description: 'Endereço completo da ocorrência' },
      ...
      { name: 'valor_fipe', type: 'number', required: false, description: 'Valor FIPE do veículo no momento do sinistro' },
      { name: 'valor_participacao', type: 'number', required: false, description: 'Valor de participação do associado' },
      { name: 'valor_orcamento', type: 'number', required: false, description: 'Valor do orçamento de reparo' },
      { name: 'valor_indenizacao', type: 'number', required: false, description: 'Valor da indenização' },
      { name: 'valor_pago', type: 'number', required: false, description: 'Valor efetivamente pago' },
      { name: 'valor_depreciacao', type: 'number', required: false, description: 'Valor da depreciação do veículo' },
      { name: 'percentual_fipe', type: 'number', required: false, description: 'Percentual da FIPE aplicado' },
      { name: 'tipo_dano', type: 'string', required: false, description: 'Tipo de dano: "parcial" ou "integral"' },
      { name: 'veiculo_recuperado', type: 'boolean', required: false, description: 'Se o veículo foi recuperado (roubo/furto)' },
      { name: 'envolvimento', type: 'string', required: false, description: '"VITIMA" ou "CAUSADOR"' },
      { name: 'data_comunicado', type: 'string (YYYY-MM-DD)', required: false, description: 'Data do comunicado do evento' },
      { name: 'solicitou_carro_reserva', type: 'boolean', required: false, description: 'Se solicitou carro reserva' },
    ],
    responseExample: {
  ...
  },
  {
    ...
    description: 'Retorna os dados de um sinistro pelo ID, com dados do associado e veículo embutidos.',
    group: 'Eventos',
    fields: [
    ...
    responseExample: {
      ...
      data_comunicado: '2026-03-25',
      associado: { id: '...', nome: 'João da Silva', cpf: '12345678901', telefone: '11999999999', email: 'joao@email.com' },
      veiculo: { id: '...', placa: 'ABC1D23', chassi: '9BWZZZ377VT004251', marca: 'Volkswagen', modelo: 'Gol', ano_fabricacao: 2022, ano_modelo: 2023, valor_fipe: 50000 },
    },
  },
  {
    id: 'list-sinistros',
    ...
    description: 'Lista sinistros com filtros por CPF do associado e status. Retorna dados do associado e veículo embutidos. Suporta paginação.',
    group: 'Eventos',
    fields: [
      { name: 'associado_cpf', type: 'string', required: false, description: 'CPF do associado para filtrar eventos (query param)' },
      { name: 'status', type: 'string', required: false, description: 'Filtrar por status: "aberto", "em_analise", "aprovado", etc. (query param)' },
      { name: 'limit', type: 'number', required: false, description: 'Quantidade de registros por página (padrão: 50, máx: 100)' },
      { name: 'offset', type: 'number', required: false, description: 'Deslocamento para paginação (padrão: 0)' },
    ],
    responseExample: {
      data: [
        {
          ...
          envolvimento: 'VITIMA',
          data_ocorrencia: '2026-03-24T15:30:00Z',
          associado: { id: '...', nome: 'João da Silva', cpf: '12345678901' },
          veiculo: { id: '...', placa: 'ABC1D23', marca: 'Volkswagen', modelo: 'Gol' },
        },
      ],
      total: 1,
      limit: 50,
