...
export function ReguladorLayout() {
  ...
  const isActive = (path: string) => {
    if (path === '/regulador') return location.pathname === '/regulador';
    return location.pathname.startsWith(path);
  };

  const getPrimeiroNome = () => {
    if (!profile?.nome) return 'Regulador';
    return profile.nome.split(' ')[0];
  };

  const getIniciais = () => {
    if (!profile?.nome) return 'R';
              ...
              {NAV_ITEMS.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className={cn('text-xs', active ? 'font-medium text-emerald-400' : 'text-slate-400')}>
                      {item.label}
                    </span>
                  </Link>
                );
