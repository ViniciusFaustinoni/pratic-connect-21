...
import { CloudRain, AlertTriangle, FileCheck, Loader2, ShieldAlert, Droplets } from 'lucide-react';
...
interface CardAnaliseAlagamentoProps {
  sinistro: {
    id: string;
...
const MOTIVOS_ANALISE = [
  { value: 'agua_salgada', label: 'Água salgada (maré/ressaca)' },
...
export function CardAnaliseAlagamento({ sinistro, associadoId }: CardAnaliseAlagamentoProps) {
  ...
  const isEmAnalise = sinistro.analise_interna === true;
  ...
  const encaminharMutation = useMutation({
    mutationFn: async () => {
      ...
      await supabase.from('sinistro_historico').insert({
        ...
        observacao: `Encaminhado para análise jurídica. Motivos: ${motivosSelecionados.map(m => MOTIVOS_ANALISE.find(ma => ma.value === m)?.label || m).join(', ')}`,
      });
    },
    onSuccess: () => {
      toast.success('Sinistro encaminhado para análise jurídica');
  ...
  return (
    <Card className={isEmAnalise ? 'border-amber-500/50' : undefined}>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <span className="text-sm text-muted-foreground">Tipo de água</span>
          <Badge variant="outline" className={
            sinistro.tipo_agua === 'salgada' 
              ? 'border-amber-500 text-amber-700' 
              : sinistro.tipo_agua === 'doce'
              ? 'border-green-500 text-green-700'
              : ''
          }>
            <Droplets className="h-3 w-3 mr-1" />
            {sinistro.tipo_agua === 'doce' ? 'Água Doce' : sinistro.tipo_agua === 'salgada' ? 'Água Salgada' : 'Não informado'}
          </Badge>
        </div>
        ...
        {/* Badge análise jurídica */}
        {isEmAnalise && (
          <div className="flex items-start gap-2 p-3 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-lg">
            <ShieldAlert className="h-4 w-4 text-amber-600 mt-0.5" />
            <div className="text-sm">
              <p className="font-medium text-amber-800 dark:text-amber-300">Em Análise Jurídica</p>
            ...
            <div className="space-y-3">
              ...
              {MOTIVOS_ANALISE.map(motivo => (
                <div key={motivo.value} className="flex items-center space-x-2">
                  <Checkbox
                    id={`alag_${motivo.value}`}
                    checked={motivosSelecionados.includes(motivo.value)}
                    onCheckedChange={() => toggleMotivo(motivo.value)}
                  />
                  <Label htmlFor={`alag_${motivo.value}`} className="text-sm cursor-pointer">
                    {motivo.label}
                  </Label>
                </div>
              ))}
              <p className="text-xs text-muted-foreground">
                Marcar encaminha para análise jurídica. O jurídico elabora parecer e pode negar.
              </p>
            </div>
